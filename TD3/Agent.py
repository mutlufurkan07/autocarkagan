...
class Agent:
    def __init__(self, gamma, tau, actorlr, criticlr, std, action_dim, mem_size, batch_size, state_dim, reward_dim,
                 noise_mag, max_action, training_or_validation):
        ...
        self.critic1 = CriticNetwork(input_dims=state_dim, action_dims=action_dim, input_out=128, layer1_dims=128,
                                     layer2_dims=64)
        self.critic1_target = CriticNetwork(input_dims=state_dim, action_dims=action_dim, input_out=128,
                                            layer1_dims=128, layer2_dims=64)
        self.critic1.load_state_dict(self.critic1_target.state_dict())

        self.critic2 = CriticNetwork(input_dims=state_dim, action_dims=action_dim, input_out=128, layer1_dims=128,
                                     layer2_dims=64)
        self.critic2_target = CriticNetwork(input_dims=state_dim, action_dims=action_dim, input_out=128,
                                            layer1_dims=128,
                                            layer2_dims=64)
    ...
    def learn(self, state_, action_, reward_, new_state_, done_):
        ...
        next_actions = self.actor_target.forward(new_state_batch).clamp(-self.max_action, self.max_action)
        Q1 = self.critic1_target(new_state_batch, next_actions)
        Q2 = self.critic2_target(new_state_batch, next_actions)
        y = torch.min(Q1, Q2)
        y = reward_batch.sum(axis=1).unsqueeze(1) + (self.gamma * y * (1 - terminal_batch.unsqueeze(1)))
    ...
    def save_models(self, episode, id):
        ...
        torch.save(self.actor.state_dict(), f"model_params_td3/{id}_{episode}_actor.pth")
        torch.save(self.actor_target.state_dict(), f"model_params_td3/{id}_{episode}_actortarget.pth")
        torch.save(self.critic1.state_dict(), f"model_params_td3/{id}_{episode}_critic.pth")
        torch.save(self.critic1_target.state_dict(), f"model_params_td3/{id}_{episode}_critictarget.pth")

    def load_models(self, episode, id):

        self.actor.load_state_dict(torch.load(f"model_params_td3/{id}_{episode}_actor.pth", map_location=self.device))
        self.actor_target.load_state_dict(
            torch.load(f"model_params_td3/{id}_{episode}_actortarget.pth", map_location=self.device))
        self.critic1.load_state_dict(
            torch.load(f"model_params_td3/{id}_{episode}_critic.pth", map_location=self.device))
        self.critic1_target.load_state_dict(
            torch.load(f"model_params_td3/{id}_{episode}_critictarget.pth", map_location=self.device))
