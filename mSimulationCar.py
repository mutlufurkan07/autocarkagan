...
class mSimulationCar:
    ...
    def setVehiclePose(self,pos_x,pos_y,rotation_z):
        self.client.simSetVehiclePose(airsim.Pose(airsim.Vector3r(pos_x , pos_y, -0.5), airsim.utils.to_quaternion(0, 0, rotation_z / 57.2957795)), True)
    ...
    def randomly_initiate_states(self):
        ...
        self.setVehiclePose(self.possible_car_states[random_index,0], self.possible_car_states[random_index,1], self.possible_car_states[random_index,2])
        # self.setVehiclePose(0, 0, 90)
        self.xx = self.possible_car_states[random_index,0]
        self.yy = self.possible_car_states[random_index,1]
        ...
        self.target_location = [self.possible_car_states[target_random_index,0],self.possible_car_states[target_random_index,1]]
...
#        plt.plot(np.array(self.timeStampsOfPosandOrientation)[:179] , diff(np.array(self.mCar_pos)[:180,0]) / diff(np.array(self.timeStampsOfPosandOrientation)[:180] ))
#        plt.show()
    ...
    def __get_carState(self,isPrint):        
        ...
        orientation_x,orientation_y,orientation_z = self.quaternion_to_eularian_angles(car_state.kinematics_estimated.orientation)
    ...
    def get_image(self):
        image1d = np.fromstring(self.RGB_image.image_data_uint8, dtype=np.uint8)
        return image1d.reshape(self.RGB_image.height, self.RGB_image.width, 3)
    ...
    def createTargetArray(self , target_pos_x,target_pos_y,curr_heading,distance_to_target):
        # target_positional_orientation = np.arctan2(target_pos_x,target_pos_y)*180/np.pi
        # target_relative_theta = (target_positional_orientation - curr_heading)
        
        # target_array = np.zeros((60,))
        
        # if (-84 <= int(target_relative_theta) <= 84):
        #     target_array[(90 + target_relative_theta)//3 -2 : (90 + target_relative_theta)//3 + 3 ] = 1 - np.ones((5,))* distance_to_target
        # # print("Target relative data" , target_relative_theta)
        # elif(-180<=int(target_relative_theta)<-85):
        #     target_array[0:6] = 1 - np.ones((5,))* distance_to_target    
        # elif ( 85<target_relative_theta<=180):
        #     target_array[55:60] = 1 - np.ones((5,))* distance_to_target    
        ...
        if target_relative_theta > 180:
            target_relative_theta -=360
        # print("positional_orşentation " , target_positional_orientation)
        target_array = np.zeros((60,))
        # print("Target relative data" , target_relative_theta)
        try:
            if(-180<int(target_relative_theta)<-83):
                ...
                target_array[int((target_relative_theta)//3)+28:int((target_relative_theta)//3)+33] = np.ones((5,))
            
            
            target_array += np.random.normal(0,0.08 , target_array.shape)
    ...
    def car_cosineReward(self,target_x,target_y,curr_x,curr_y,curr_heading):
        ...
        return (np.dot(heading_vec , target_vector)) / (np.linalg.norm(target_vector) * np.linalg.norm(heading_vec)) #reward_input   
    ...
    def take_action_and_get_current_lidar_and_targetArray(self,TARGET_POS_X,TARGET_POS_Y,target_initial_distance):
        ...
        lidar_data_sampled = car_pos_lidar_data[4:len(car_pos_lidar_data) - 1]#[np.arange(1,180,3)] #lidar data sampled from 180 to 60
        distance_to_target = np.sqrt((TARGET_POS_X-curr_x)**2 + (TARGET_POS_Y - curr_y)**2)
        car_target_array_data = self.createTargetArray(TARGET_POS_X - curr_x, TARGET_POS_Y - curr_y, curr_heading, distance_to_target/target_initial_distance)
    ...
    def choose_steering_angle_from_agent(self , act):
        new_act = 0
        if act == 0: new_act = -0.75
        elif act == 1: new_act = 0
        elif act == 2: new_act = 0.75
        return new_act
