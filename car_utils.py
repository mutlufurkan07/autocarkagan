...
print("Starting")
agent = Agent(gamma, tau, actorlr, criticlr, variance, action_dim, int(mem_size), batch_size, state_dim, reward_dim , eps_dec)
...
for epoch in range(int(MAX_EPOCH)):    
    ...
    state_numpy , state_torch_tensor , curr_x, curr_y , curr_heading , isCollidedFlag ,isClear_flag =  mCar.take_action_and_get_current_lidar_and_targetArray(TARGET_POS_X,TARGET_POS_Y,target_initial_distance)
    ...
    while 1:        
        ...
        new_state_numpy , new_state_torch_tensor , new_curr_x, new_curr_y , new_curr_heading ,isCollidedFlag , isClear_flag_new =  mCar.take_action_and_get_current_lidar_and_targetArray(TARGET_POS_X,TARGET_POS_Y,target_initial_distance)
        new_state_torch_tensor = torch.cat((new_state_torch_tensor, torch.tensor(steering_que)))
        new_state_torch_tensor.to(device)
        new_current_cosine_reward = mCar.car_cosineReward(TARGET_POS_X, TARGET_POS_Y, new_curr_x, new_curr_y, new_curr_heading)
        ...
        #update network       
        reward_tensor = torch.tensor([-0.075, current_cosine_reward / 2, (isCollidedFlag or time_pounishmet_flag) * (-200), success*400 , temp_circle_reward*25 , isClear_flag*-1])        
        ...
        if (time_pounishmet_flag or  isCollidedFlag or success):            
            ...
            tot_step += step_len
            print(f"Epoch: {epoch} , Reward: {epoch_reward.item():.3f} , Avg Reward: {avg_reward.item():.3f}  , Success: {success} , Step Len: {step_len}  , Total Steps: {tot_step}")
            ...
            mCar.client.simPlotPoints(points = [Vector3r(TARGET_POS_X,TARGET_POS_Y,-2)], color_rgba=[1, 0.0, 0.0, 1.0], size = 150, duration = 10, is_persistent = True)
    ...
    if training_flag:        
        ...
        f.write("Epoch num" + str(epoch) + "   Step_len: "+ str(step_len) +"   Epoch Reward: " + str(epoch_reward.item()) + " Memory index: " + str(agent.memory.mem_index) + "Success: " + str(success)   +" Time since beginning " + str(elapsed_since_training_start) +   "\n")
        f.close()
